...
function Rsq_Pre_output=MultipleRegFunc(Input_dynamics,Output_Dynamics,Figures)
...
if Figures==1
    ...
    if Lag_Number~=0
        figure;
        suptitle(['; WLag:',num2str(Lag_Number)]);
    elseif Lag_Number==0
        figure;
        suptitle(['W/OLag'])
...
%      ylim([0,300])
    ...
    if Lag_Number~=0
        title(['WLag:',num2str(Lag_Number),'; Predicted Output Using CV: R2: ',num2str(Rsq_Pre_output)])
    elseif Lag_Number==0
        title(['Without Lag','; Predicted Output Using CV: R2: ',num2str(Rsq_Pre_output)])
